...
from database import db, ChargeData, PositionData, Scooter
from utils import *
...
def post_charge_data(scooter_id: int) -> tuple[Response, int]:
    data = request.get_json()
    db.session.add(ChargeData(scooter_id=scooter_id, charge=data['charge']))
    db.session.commit()

    return jsonify({'message': 'Scooters charge data updated successfully'}), 200
...
def post_position_data(scooter_id: int) -> tuple[Response, int]:
    data = request.get_json()
    db.session.add(PositionData(scooter_id=scooter_id, latitude=data['latitude'], longitude=data['longitude']))
    db.session.commit()

    return jsonify({'message': 'Scooters position data updated successfully'}), 200
...
# other
@bp.route('/scooters/<int:scooter_id>', methods=['DELETE'])  # FIXME: cascade delete
